...
> 我把前缀难度曲线算出来了，是干净的单调下降：
>
> | 前缀样本数 | 50 | 100 | 200 | 300 |
> |---|---|---|---|---|
> | cross-encoder Recall@5 | **46.0%** | 40.5% | 39.5% | **38.5%** |
>
> 前 50 条确实显著更容易（46% vs 38.5%）；而且 50 条那次检索池只有 1000 段，
> 这次是 5650 段，**池子更大也意味着更难**。两个因素叠加，
> 就是'48% 变 38.5%'的完整解释。
>
> 但**差量是稳的**：四个前缀上的重排增益分别是 +9.0 / +11.0 / +10.25 / +11.67pp——
> 说明'重排有用'这个结论不随样本集变化，而**绝对指标的绝对值必须先看样本量**。
> 所以我现在把这条曲线写进报告，立了一条规则：
> **绝对指标只能用它自己的样本量对应的那一行。**」
