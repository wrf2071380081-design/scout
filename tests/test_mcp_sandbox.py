...
def test_sandbox_runs_pure_computation() -> None:
    # 显式给一个宽松超时。这条用例验证的是"拦截 → 执行 → 输出截断"这条链路，
    # 不是在测解释器启动速度；本机子进程冷启动偶尔超过默认的 5s（杀软扫描/负载），
    # 用一个贴近真实的超时，才是这条用例本该有的语义。
    output = run_python("print(1 + 2 * 3)", timeout_seconds=60.0)
    assert "7" in output
    # 环境备注：子进程偶尔会在退出时碰上原生库卸载崩溃（0xC0000409 退出码）。
    # 这与沙箱工具本身无关——输出是正确的。因此断言接受这两种退出码，
    # 但绝不接受其它失败。
